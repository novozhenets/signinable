...
Call signinable in an ActiveRecord class and/or pass options to make your model token signinable. 
...
+restriction+ can be passed as array of parameters, which have to be checked every time user tries to +authenticate_with_token+ or +signout+. 
This is done to prevent unauthorized access to tokens or to forbid using one token from different IPs or browsers. Posibble values are: +ip+, +user_agent+ and +referer+ <b>Default</b> is empty array.
